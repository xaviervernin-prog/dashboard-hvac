...
TABLE = "interventions"
VALID_STATUTS = {"planifie", "en_cours", "termine", "annule"}
...
class AgendaService:
    ...
    def list(
        ...
        query = self.db.table(TABLE).select(
            "*, clients(nom, entreprise), devis(numero, objet)"
        )
        if date_debut:
    ...
    def get(self, intervention_id: UUID) -> dict:
        response = (
            self.db.table(TABLE)
            .select("*, clients(nom, entreprise), devis(numero, objet)")
    ...
    def create(self, payload: InterventionCreate) -> dict:
        ...
        if payload.date_fin and payload.date_fin <= payload.date_debut:
        ...
        if payload.date_fin:
            data["date_fin"] = payload.date_fin.isoformat()

        response = self.db.table(TABLE).insert(data).execute()
        logger.info("Intervention créée : %s", response.data[0].get("id"))
        return response.data[0]
